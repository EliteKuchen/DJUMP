...
bool CHARACTER::is_grounded()
{
	...
	if(col_check_platte((int)(pos.x-phys_size/2), (int)(pos.y+phys_size/2+5)))
		return true;
	if(col_check_platte((int)(pos.x-phys_size/2), (int)(pos.y+phys_size/2+5)))
		return true;
...
int CHARACTER::handle_ninja()
{
	...
	{
		// Set player velocity
		core.vel = ninja.activationdir * data->weapons.ninja.velocity * 3;//EliteKuchen edit
		vec2 oldpos = pos;
	...
	switch(active_weapon)
	{
		...
		case WEAPON_GRENADE:
		{
			PROJECTILE *proj = new PROJECTILE(WEAPON_GRENADE,
				...
				1, PROJECTILE::PROJECTILE_FLAGS_EXPLODE, 0, SOUND_GRENADE_EXPLODE, WEAPON_GRENADE, false);
...
void CHARACTER::tick()
{
	...
	if(hoehe > player->punkte)//Punkte Geben
	{
		player->punkte = hoehe;
		leben = config.sv_leben;
	}	
	...
	if(config.sv_autojump && is_grounded() && core.jumped == 0 &&
		col_get_index(pos.x, pos.y) != TILE_NOJUMP)
		input.jump = 1;
	...
	{
		str_format(buftime, sizeof(buftime), "Current Points: %d", player->punkte);
		
		game.send_broadcast(buftime, player->client_id);

		refreshtime = server_tick()/50;
	}
	...
	else if((col_get_index(pos.x, pos.y) == TILE_END || leben <= 0) && race_state == RACE_STARTED)
	{
		char buf[128];
		str_format(buf, sizeof(buf), "%s died : %d points", server_clientname(player->client_id), player->punkte);
		game.send_chat(-1,GAMECONTEXT::CHAT_ALL, buf);
		
		PLAYER_SCORE *pscore = ((GAMECONTROLLER_RACE*)game.controller)->score.search_score(player->client_id, 0, 0);
		if(pscore && player->punkte > pscore->points)
		{
			str_format(buf, sizeof(buf), "New record: %d point(s) better", player->punkte - pscore->points);
			game.send_chat(-1, GAMECONTEXT::CHAT_ALL, buf);
		}
		...
		if(strncmp(server_clientname(player->client_id), "nameless tee", 12) != 0)
			((GAMECONTROLLER_RACE*)game.controller)->score.parsePlayer(player->client_id, player->punkte);
	...
	else if(core.vel.y > -2)
	{
		for(int i = -25; i<=25; i++)
		{
			i += core.pos.x;
		 	if(col_get_index(i, core.pos.y - 15) == TILE_PLATTE)
				core.pos.y += tuning.gravity;
		 	else if(col_get_index(i, core.pos.y) == TILE_PLATTE)
				core.pos.y += tuning.gravity;
		 	else if(col_get_index(i, core.pos.y + 10) == TILE_PLATTE)
				core.pos.y += tuning.gravity;
			i -= core.pos.x;
		}
	}
